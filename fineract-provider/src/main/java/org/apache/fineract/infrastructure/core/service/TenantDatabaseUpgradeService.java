...
public class TenantDatabaseUpgradeService {
    ...
    private void upgradeTenantDB() {
        LOG.info("upgradeTenantDB: FINERACT_DEFAULT_TENANTDB_HOSTNAME = {}, FINERACT_DEFAULT_TENANTDB_PORT = {}", datasourceHost,
                datasourcePort);

        final Flyway flyway = Flyway.configure().dataSource(tenantDataSource).locations("sql/migrations/list_db").outOfOrder(true)
                // FINERACT-773
                .placeholders(Map.of("fineract_default_tenantdb_hostname", datasourceHost, "fineract_default_tenantdb_port", datasourcePort,
                        "fineract_default_tenantdb_uid", datasourceUsername, "fineract_default_tenantdb_pwd", datasourcePassword))
